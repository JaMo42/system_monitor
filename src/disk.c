...
DiskInit(WINDOW *win) {
    DiskDrawBorder(win);
    disk_canvas = CanvasCreateSized(ceil(disk_radius) + 1, ceil(disk_radius / 2));
    if (!(disk_fs || (disk_fs = getenv("SM_DISK_FS"))) || !*disk_fs) {
        disk_fs = "/";
    }
...
DiskUpdate() {
    ...
    list_for_each(disk_filesystems, it) {
        ...
        fs_info->used = used;
        fs_info->changed =
            (fs_info->total != before.total || fs_info->avail != before.avail ||
...
static inline void
DiskDrawArc(double x, double y, double inner_radius,
            double outer_radius, double begin_angle, double end_angle, short color) {
    for (double angle = begin_angle; angle < end_angle; angle += 0.02) {
        CanvasDrawLine(disk_canvas, cos(angle) * inner_radius + x,
                       sin(angle) * inner_radius + y, cos(angle) * outer_radius + x,
                       sin(angle) * outer_radius + y, color);
    }
}
