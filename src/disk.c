...
DiskInit(WINDOW *win) {
    DiskDrawBorder(win);
    disk_canvas
        = CanvasCreateSized(ceil(disk_radius) + 1, ceil(disk_radius / 2));
...
DiskDrawFSInfo(WINDOW *win, Disk_FS_Info *fs_info, int x, int y) {
    ...
    DiskDrawArc(
        cx, cy, inner_radius, outer_radius, fill, end, theme->disk_free
    );
    DiskDrawArc(
        cx, cy, inner_radius, outer_radius, start, fill, theme->disk_used
    );
    wattron(win, A_BOLD);
...
DiskResize(WINDOW *win) {
    ...
    if (space < min_padding) {
        ...
        disk_margin = space;
        disk_padding
            = ((total - content + disk_filesystems->count)
